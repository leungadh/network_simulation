#!/usr/bin/env python3
"""
Generate the Stage 1 Grafana dashboard.
...
timeseries("Sessions per minute",
           ...
           "decay appear here once Stage 2 adds time-of-day curves.",
...
timeseries("Bytes by application",
           ...
           "a single SSL series; Stage 2's distinct hostnames give SNI "
...
dashboard = {
    "uid": "netsim-stage1",
    "title": "NetSim — Stage 1 telemetry",
...
with open("netsim-stage1.json", "w") as fh:
    json.dump(dashboard, fh, indent=2)

print(f"netsim-stage1.json written: {len(panels)} panels")
for p in panels:
    g = p["gridPos"]
