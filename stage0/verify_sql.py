#!/usr/bin/env python3
"""
Stage 1 exit-criteria check, scored from ClickHouse.

Same criteria as Stage 0, but measured against `netsim.labelled_flows` — the
...
def main():
    print("\nStage 1 exit criteria — scored from ClickHouse\n" + "=" * 56)
    ...
    check(f"sustained run ({mins:.1f} min)", mins >= 30, warn=True,
          hint="Stage 1 wants ≥30 minutes to call the join rate trustworthy.\n"
               "Set DURATION_S=1800 in .env and re-run. Not fatal — the other\n"
               "checks are still meaningful on a short run.")

    print("\n" + "=" * 56)
    if all(results):
        print(f"{GREEN}Stage 1 complete.{RESET} Telemetry spine verified end to end.\n")
        print(f"  Grafana: http://localhost:{os.environ.get('GRAFANA_PORT', '3000')}\n")
        return 0
    print(f"{RED}Stage 1 not met.{RESET}\n")
    return 1
