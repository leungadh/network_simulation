...
Anything short of all five passing means Stage 2 is not safe to start.
"""
...
def main():
    ...
    if join_rate < 98:
        detail = (f"{matched}/{len(closes)} sessions matched an intent.\n"
                  ...
                  "`show security nat source summary` — Stage 1 should have none), or\n"
    ...
    print("\n" + "=" * 50)
    if all(results):
        print(f"{GREEN}Stage 1 complete.{RESET} Safe to start Stage 2.\n")
        return 0
    print(f"{RED}Stage 1 not met.{RESET} Fix the failures above before Stage 2 —\n"
